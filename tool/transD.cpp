...
void load() {
    ...
    FILE *fin;
    INT tmp;
    fin = fopen((loadPath + "entity2vec" + note + ".vec_D").c_str(), "r");
    for (INT i = 0; i < entityTotal; i++) {
    ...
    fclose(fin);
    fin = fopen((loadPath + "relation2vec" + note + ".vec_D").c_str(), "r");
    for (INT i = 0; i < relationTotal; i++) {
    ...
    fin = fopen((loadPath + "A" + note + ".vec_D").c_str(), "r");
    for (INT i=0; i < relationTotal; i++) {
...
void out() {
		...
		FILE* f2 = fopen((outPath + "relation2vec" + note + ".vec_D").c_str(), "w");
		FILE* f3 = fopen((outPath + "entity2vec" + note + ".vec_D").c_str(), "w");
		for (INT i=0; i < relationTotal; i++) {
		...
		fclose(f2);
		fclose(f3);
		FILE* f1 = fopen((outPath + "A" + note + ".vec_D").c_str(),"w");
		for (INT i = 0; i < relationTotal; i++) {
			INT last = dimensionR * i;
			for (INT ii = 0; ii < dimensionR; ii++)
