...
def REINFORCE(training_pairs, policy_nn, num_episodes):
	...
	for i_episode in range(num_episodes):
		start = time.time()
		print_title('Episode %d' % i_episode)
		print_status('Training sample: ', train[i_episode][:-1])
		...
		if len(state_batch_negative) != 0:
			print_status('Penalty to invalid steps:', len(state_batch_negative))
			policy_nn.update(np.reshape(state_batch_negative, (-1, state_dim)), -0.05, action_batch_negative)

		print_status('----- FINAL PATH -----')
		print_status('\t'.join(env.path))
		print_status('PATH LENGTH', len(env.path))
		print_status('----- FINAL PATH -----')
		...
		if done == 1:
			...
			# 	print_status('diverse_reward', diverse_reward)
			...
			print_status('Failed, Do one teacher guideline')
			try:
				...
				print_status('Teacher guideline failed')
		print_status('Episode time: ', time.time() - start)
		print_status('\n')
	print_subtitle('Success percentage:', success/num_episodes)
...
def retrain():
	...
	with tf.Session() as sess:
		...
		print_status("sl_policy restored")
...
def test():
			...
			for t in count():
				...
				if done or t == max_steps_test:
					if done:
						...
						print_status('Episode ends due to step limit\n')
					break
				state_idx = new_state
	...
	print_subtitle('Success persentage:', success/test_num)

	f = open(dataPath + 'tasks/' + relation + '/' + 'path_to_use.txt', 'w')
	for item in ranking_path:
		f.write(item[0] + '\n')
	f.close()
	print_status('path to use saved')
	return
