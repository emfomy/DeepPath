...
void load() {
	...
	FILE *fin;
	INT tmp;
	fin = fopen((loadPath + "entity2vec" + note + ".unif").c_str(), "r");
	for (INT i = 0; i < entityTotal; i++) {
	...
	fclose(fin);
	fin = fopen((loadPath + "relation2vec" + note + ".unif").c_str(), "r");
	for (INT i = 0; i < relationTotal; i++) {
		INT last = i * dimension;
		for (INT j = 0; j < dimension; j++)
...
void out() {
		...
		FILE* f2 = fopen((outPath + "relation2vec" + note + ".unif").c_str(), "w");
		FILE* f3 = fopen((outPath + "entity2vec" + note + ".unif").c_str(), "w");
		for (INT i=0; i < relationTotal; i++) {
