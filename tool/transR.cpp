...
void init() {
	...
	if (initPath != "") {
		...
		FILE* f1 = fopen((initPath + "entity2vec" + note1 + ".bern").c_str(),"r");
		for (INT i = 0; i < entityTotal; i++) {
			for (INT ii = 0; ii < dimension; ii++)
		...
		fclose(f1);
		FILE* f2 = fopen((initPath + "relation2vec" + note1 + ".bern").c_str(),"r");
		for (INT i=0; i < relationTotal; i++) {
			for (INT ii=0; ii < dimension; ii++)
...
void load() {
    ...
    FILE *fin;
    INT tmp;
    fin = fopen((loadPath + "entity2vec" + note + ".bern").c_str(), "r");
    for (INT i = 0; i < entityTotal; i++) {
    ...
    fclose(fin);
    fin = fopen((loadPath + "relation2vec" + note + ".bern").c_str(), "r");
    for (INT i = 0; i < relationTotal; i++) {
    ...
    fin = fopen((loadPath + "A" + note + ".bern").c_str(), "r");
    for (INT i = 0; i < relationTotal; i++)
            for (INT jj = 0; jj < dimension; jj++)
                for (INT ii = 0; ii < dimensionR; ii++)
...
void out() {
		...
		FILE* f2 = fopen((outPath + "relation2vec" + note + ".bern").c_str(), "w");
		FILE* f3 = fopen((outPath + "entity2vec" + note + ".bern").c_str(), "w");
		for (INT i = 0; i < relationTotal; i++) {
		...
		fclose(f2);
		fclose(f3);
		FILE* f1 = fopen((outPath + "A" + note + ".bern").c_str(),"w");
		for (INT i = 0; i < relationTotal; i++)
			for (INT jj = 0; jj < dimension; jj++) {
				for (INT ii = 0; ii < dimensionR; ii++)
